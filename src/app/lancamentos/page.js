...
export default function Lancamentos() {
  ...
  const handleSalvar = async (e) => {
    ...
    try {
      ...
      if (res.ok) {
        await carregarDados();
        fecharModal();
        exibirNotificacao(modoEdicao ? "Atualizado!" : "Agendado!");
      }
    } catch (error) { exibirNotificacao("Erro no processo", "erro"); }
  ...
  const handleExcluir = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        exibirNotificacao("Removido!", "sucesso");
      }
  ...
  const handleEfetivarBaixa = async (e) => {
    ...
    try {
      ...
      if (res.ok) {
        await carregarDados();
        setModalPagamento({ ...modalPagamento, aberto: false });
        exibirNotificacao("Liquidação concluída!");
      }
  ...
  const abrirEdicao = (item) => {
    setNovoItem({ ...item, valor: item.valor.toString() });
    setModoEdicao(true);
    setModalAberto(true);
  };
  ...
  const fecharModal = () => {
    ...
    setModoEdicao(false);
    setNovoItem({ descricao: '', valor: '', tipo: 'SAIDA', categoria: 'OUTROS', tipoConta: 'PJ' });
  };

  // LÓGICA DE BUSCA: Filtra pelo nome do título (descrição)
  ...
  return (
    <div className="relative space-y-6 max-w-[1600px] mx-auto p-4 pb-20 font-sans tracking-tight">
      ...
      <div className="flex flex-col lg:flex-row justify-between items-center gap-6">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-slate-400 text-[10px] font-black uppercase mt-1 tracking-widest">Controle de Fluxo Operacional</p>
          </div>
        </div>
        
        <div className="flex gap-3 w-full lg:w-auto items-center">
          {/* CAMPO DE BUSCA REATIVADO */}
          <div className="relative flex-1 lg:w-80 group">
            ...
            <input 
              ...
              className="w-full pl-12 pr-4 py-4 bg-white border border-slate-200 rounded-2xl text-sm font-bold outline-none focus:ring-4 focus:ring-blue-50 transition-all shadow-sm placeholder:text-slate-300" 
            />
          </div>
          <button onClick={() => setModalAberto(true)} className="flex items-center gap-3 bg-slate-900 hover:bg-blue-600 text-white px-8 py-4 rounded-2xl font-black uppercase text-[11px] tracking-widest transition-all shadow-xl active:scale-95 whitespace-nowrap"><Plus size={18} /> Agendar Título</button>
        </div>
      </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left">
            <thead className="bg-slate-50/50 border-b border-slate-100 text-[10px] font-black text-slate-400 uppercase tracking-widest">
              <tr>
                <th className="p-8">Descrição / Info</th>
                <th className="p-8 text-center">Composição</th>
                <th className="p-8 text-center">Status / Ação</th>
                <th className="p-8 text-right">Valor Bruto</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-50">
              {itensDaPagina.length === 0 && !isLoading ? (
                <tr><td colSpan="4" className="p-20 text-center text-slate-300 font-black uppercase text-xs tracking-widest">Nenhum lançamento correspondente</td></tr>
              ) : itensDaPagina.map((item) => {
                const isEntrada = item.tipo === 'ENTRADA';
                return (
                  <tr key={item.id} className={`hover:bg-slate-50/40 transition-all ${isEntrada ? 'bg-emerald-50/10' : 'bg-rose-50/10'}`}>
                    <td className="p-8">
                      <div className="flex flex-col gap-1">
                        <span className="font-black text-slate-800 uppercase text-sm">{item.descricao}</span>
                        <div className="flex items-center gap-2">
                           <span className="text-[9px] font-black text-slate-400 uppercase bg-slate-100 px-2 py-0.5 rounded-md flex items-center gap-1"><Tag size={10}/> {item.categoria}</span>
                           {item.status === 'PAGO' && <span className="text-[9px] font-black text-blue-400 uppercase bg-blue-50 px-2 py-0.5 rounded-md flex items-center gap-1"><Landmark size={10}/> {item.banco}</span>}
                        </div>
                      </div>
                    </td>
                    ...
                    <td className="p-8 text-center">
                      <div className="flex items-center justify-center gap-3">
                        {item.status !== 'PAGO' ? (
                          <>
                            <button onClick={() => abrirEdicao(item)} className="p-3 text-slate-300 hover:text-blue-600 hover:bg-blue-50 rounded-2xl transition-all"><Edit3 size={18}/></button>
                            <button onClick={() => setModalExcluir({ aberto: true, id: item.id })} className="p-3 text-slate-300 hover:text-rose-600 hover:bg-rose-50 rounded-2xl transition-all"><Trash2 size={18}/></button>
                            <button 
                              onClick={() => setModalPagamento({ ...modalPagamento, aberto: true, item })} 
                              className="ml-2 bg-emerald-600 text-white px-6 py-2.5 rounded-xl text-[10px] font-black uppercase hover:bg-slate-900 transition-all shadow-lg flex items-center gap-2 active:scale-95"
                            ><Calendar size={14}/> Liquidar</button>
                          </>
                        ) : (
                          <div className="flex flex-col items-center">
                            <div className="flex items-center gap-2 bg-emerald-500 text-white px-4 py-2 rounded-xl text-[10px] font-black uppercase shadow-lg shadow-emerald-100">
                              <CheckCircle2 size={14}/> Pago em {new Date(item.data).toLocaleDateString('pt-BR')}
                            </div>
                            <span className="text-[8px] font-black text-slate-400 mt-1.5 uppercase tracking-widest">{item.formaPagamento}</span>
                          </div>
                        )}
                      </div>
                    </td>
                    <td className={`p-8 text-right font-black text-base tracking-tighter ${isEntrada ? 'text-emerald-600' : 'text-rose-600'}`}>
                      {isEntrada ? '+ ' : '- '}
      ...
      {/* MODAL AGENDAR (Foco no COMPROMISSO) */}
      {modalAberto && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4 animate-in zoom-in-95 duration-200">
          <div className="bg-white w-full max-w-lg rounded-[3rem] shadow-2xl overflow-hidden border border-slate-100">
            <div className="p-8 bg-slate-50 border-b border-slate-100 flex justify-between items-center text-slate-800">
              <h3 className="text-xl font-black uppercase tracking-tighter italic">Agendar Novo Título</h3>
              <button onClick={fecharModal} className="p-2 hover:bg-slate-200 rounded-full transition-all"><X size={24}/></button>
            </div>
            <form onSubmit={handleSalvar} className="p-10 space-y-6">
              <div>
                ...
                <input type="text" required placeholder="Ex: NF 1234 - Manutenção" className="w-full p-4 bg-slate-50 rounded-2xl border border-slate-100 font-bold outline-none focus:ring-4 focus:ring-blue-50 transition-all" value={novoItem.descricao} onChange={e => setNovoItem({...novoItem, descricao: e.target.value})} />
              </div>
              
              <div>
                <label className="text-[10px] font-black text-slate-400 uppercase mb-2 block tracking-widest ml-1">Valor Bruto (R$)</label>
                <input type="number" step="0.01" required placeholder="0,00" className="w-full p-4 bg-slate-50 rounded-2xl border border-slate-100 font-black text-lg outline-none focus:ring-4 focus:ring-blue-50" value={novoItem.valor} onChange={e => setNovoItem({...novoItem, valor: e.target.value})} />
              </div>

              <div className="grid grid-cols-2 gap-4">
                <div className="flex bg-slate-100 p-1 rounded-2xl">
                  <button type="button" onClick={() => setNovoItem({...novoItem, tipo: 'ENTRADA'})} className={`flex-1 py-3 rounded-xl text-[10px] font-black transition-all ${novoItem.tipo === 'ENTRADA' ? 'bg-emerald-500 text-white shadow-md' : 'text-slate-400'}`}>RECEITA</button>
                  <button type="button" onClick={() => setNovoItem({...novoItem, tipo: 'SAIDA'})} className={`flex-1 py-3 rounded-xl text-[10px] font-black transition-all ${novoItem.tipo === 'SAIDA' ? 'bg-rose-500 text-white shadow-md' : 'text-slate-400'}`}>DESPESA</button>
                </div>
                <div className="flex bg-slate-100 p-1 rounded-2xl">
              ...
              <select className="w-full p-4 bg-slate-50 rounded-2xl border border-slate-100 font-bold text-sm outline-none appearance-none" value={novoItem.categoria} onChange={e => setNovoItem({...novoItem, categoria: e.target.value})}>
              ...
              <button type="submit" disabled={isSalvando} className="w-full bg-slate-900 text-white p-6 rounded-[1.5rem] font-black uppercase text-xs tracking-widest hover:bg-blue-600 transition-all shadow-xl active:scale-95 disabled:opacity-50">
                {isSalvando ? <Loader2 className="animate-spin mx-auto"/> : "Confirmar Agendamento"}
              </button>
            </form>
          </div>
        </div>
      ...
      {/* MODAL LIQUIDAR (Foco na BAIXA REAL) */}
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4 animate-in zoom-in-95">
          <div className="bg-white w-full max-w-lg rounded-[3rem] shadow-2xl overflow-hidden border border-slate-100">
            <div className="p-8 bg-emerald-600 text-white flex justify-between items-center italic">
              <h3 className="text-xl font-black uppercase tracking-tighter flex items-center gap-3"><DollarSign size={24}/> Liquidar Compromisso</h3>
            ...
            <form onSubmit={handleEfetivarBaixa} className="p-10 space-y-5">
               <div className="bg-slate-50 p-6 rounded-3xl border border-slate-100 mb-2">
                  <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Título em aberto</p>
               ...
               <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="text-[10px] font-black text-slate-400 uppercase mb-2 block tracking-widest ml-1 italic">Data de Baixa</label>
                  ...
                  <div>
                    <label className="text-[10px] font-black text-slate-400 uppercase mb-2 block tracking-widest ml-1">Meio Pagto</label>
                    <div className="relative">
      ...
      {modalExcluir.aberto && (
        <div className="fixed inset-0 z-[150] flex items-center justify-center bg-slate-900/80 backdrop-blur-sm p-4 animate-in fade-in duration-300">
          <div className="bg-white w-full max-sm rounded-[2.5rem] p-10 text-center shadow-2xl border border-slate-100">
