...
export default function Lancamentos() {
  ...
  const handleSalvar = async (e) => {
    ...
    try {
      const res = await fetch(url, {
        ...
        body: JSON.stringify({ ...novoItem, parcelas: parseInt(novoItem.parcelas) })
      });
      if (res.ok) {
        await carregarDados();
  ...
  const abrirEdicao = (item) => {
    setNovoItem({ ...item, valor: item.valor.toString(), data: item.data.split('T')[0] });
    setModoEdicao(true);
    setModalAberto(true);
  };
  ...
  const confirmarExclusao = async () => {
    ...
    try {
      ...
      if (res.ok) {
        // Remove visualmente todas as parcelas que começam com o mesmo nome para Cascade visual
        setTransacoes(prev => prev.filter(item => !item.descricao.startsWith(baseName)));
        exibirNotificacao("Registro(s) removido(s)!");
      }
  ...
  return (
    <div className="relative space-y-6 animate-in fade-in pb-10 p-4 md:p-8 bg-slate-50 min-h-screen font-sans">
      ...
      <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-6">
        ...
        <div className="flex gap-3 w-full lg:w-auto">
          ...
          <button onClick={() => { setModoEdicao(false); setModalAberto(true); }} className="flex items-center gap-2 bg-slate-900 hover:bg-blue-600 text-white px-6 py-3 rounded-xl font-black uppercase text-[10px] tracking-widest transition-all shadow-xl"><Plus size={16} /> Novo Registro</button>
        </div>
      </div>
          ...
          <table className="w-full text-left">
            ...
            <tbody className="divide-y divide-slate-50">
              {itensDaPagina.map((item) => {
                ...
                // Lógica de Soma Dinâmica: Busca todas as parcelas do grupo no estado local
                ...
                return (
                  <React.Fragment key={item.id}>
                    <tr className={`transition-all hover:bg-white ${corFundo}`}>
                      ...
                      <td className="p-6 text-center">
                        <div className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-[9px] font-black uppercase border ${pagas === item.totalParcelas && item.totalParcelas > 0 ? 'bg-emerald-600 text-white' : 'bg-blue-600 text-white'}`}>
                           {item.totalParcelas > 1 ? <Layers size={10} /> : <CheckCircle2 size={10}/>} {pagas}/{item.totalParcelas || 1} PAGAS
                        </div>
                      </td>
                      <td className="p-8 text-right">
                        ...
                        {item.totalParcelas > 1 && <div className="text-[9px] font-bold text-slate-400 uppercase italic">Parcelas de {Number(item.valor).toLocaleString('pt-BR')}</div>}
                      </td>
                      <td className="p-6 text-center">
                        <div className="flex items-center justify-center gap-2">
                           <button onClick={() => abrirEdicao(item)} className="p-2 text-blue-500 hover:bg-blue-50 rounded-lg transition-all"><Edit3 size={18}/></button>
                    ...
                    {estaAberto && parcelasDoGrupo.map((parc) => (
                      <tr key={parc.id} className="bg-slate-50/80 animate-in slide-in-from-top-1 border-l-4 border-blue-500">
                        ...
                        <td className="p-4 text-center"><span className="text-[9px] font-black text-slate-300">PARCELA {parc.parcelaAtual}</span></td>
                        <td className="p-4 text-center">
                            <button disabled={processandoId === parc.id} onClick={() => handleAlternarStatus(parc)} className={`px-3 py-1 rounded-lg text-[8px] font-black uppercase shadow-sm ${parc.status === 'PAGO' ? 'bg-emerald-600 text-white' : 'bg-blue-500 text-white'}`}>
                                {processandoId === parc.id ? <Loader2 size={10} className="animate-spin" /> : parc.status}
                            </button>
                        </td>
                        <td className="p-4 text-right">
                          <div className="flex flex-col items-end">
                            <span className="font-bold text-xs text-slate-700">{Number(parc.valor).toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}</span>
                            <button onClick={() => abrirEdicao(parc)} className="text-[8px] font-black text-blue-500 uppercase hover:underline">Ajustar Valor</button>
                          </div>
                        </td>
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4 animate-in zoom-in-95">
          <div className="bg-white w-full max-w-2xl rounded-[2.5rem] shadow-2xl border border-slate-100 overflow-hidden flex flex-col">
            <div className="flex justify-between items-center p-8 bg-slate-50 border-b border-slate-100">
              <h3 className="text-xl font-black text-slate-800 uppercase tracking-tighter italic">{modoEdicao ? 'Editar Registro' : 'Novo Lançamento'}</h3>
            ...
            <form onSubmit={handleSalvar} className="p-8 space-y-6 overflow-y-auto">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="md:col-span-2">
                  <label className="text-[10px] font-black text-slate-400 uppercase mb-1.5 ml-1">Descrição</label>
                ...
                <div>
                  <label className="text-[10px] font-black text-slate-400 uppercase mb-1.5 ml-1">Data / Vencimento</label>
                ...
                <div>
                  <label className="text-[10px] font-black text-slate-400 uppercase mb-1.5 ml-1">Valor (R$)</label>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 p-6 bg-slate-50 rounded-[2.5rem] border border-slate-100">
                <div>
                  <label className="text-[10px] font-black text-blue-600 uppercase mb-1.5 ml-1">Banco</label>
                  <select value={novoItem.banco} onChange={e => setNovoItem({...novoItem, banco: e.target.value})} className="w-full p-3 bg-white border border-slate-200 rounded-xl font-bold text-sm outline-none cursor-pointer">
                    {bancos.map(b => <option key={b} value={b}>{b}</option>)}
                  </select>
                </div>
                {!modoEdicao && (
                  <div>
                    <label className="text-[10px] font-black text-blue-600 uppercase mb-1.5 ml-1">Parcelas</label>
                    <input type="number" min="1" value={novoItem.parcelas} onChange={e => setNovoItem({...novoItem, parcelas: e.target.value})} className="w-full p-3 bg-white border border-slate-200 rounded-xl font-black text-blue-600 text-sm outline-none" />
                  </div>
                )}
                <div className="md:col-span-2 pt-2">
                    <div className="flex bg-white p-1 rounded-xl border border-slate-200 shadow-sm">
                      <button type="button" onClick={() => setNovoItem({...novoItem, tipoConta: 'PF'})} className={`flex-1 py-3 rounded-lg text-[10px] font-black flex items-center justify-center gap-2 transition-all ${novoItem.tipoConta === 'PF' ? 'bg-orange-500 text-white shadow-md' : 'text-slate-400 hover:bg-slate-50'}`}>PESSOA FÍSICA</button>
                      <button type="button" onClick={() => setNovoItem({...novoItem, tipoConta: 'PJ'})} className={`flex-1 py-3 rounded-lg text-[10px] font-black flex items-center justify-center gap-2 transition-all ${novoItem.tipoConta === 'PJ' ? 'bg-blue-600 text-white shadow-md' : 'text-slate-400 hover:bg-slate-50'}`}>PESSOA JURÍDICA</button>
                    </div>
                </div>
              </div>

              <button type="submit" disabled={isSalvando} className="w-full bg-slate-900 text-white p-6 rounded-[1.5rem] font-black uppercase text-xs tracking-widest hover:bg-blue-600 transition-all shadow-xl flex items-center justify-center gap-2">
      ...
      {modalConfirmacao.aberto && (
        <div className="fixed inset-0 z-[150] flex items-center justify-center bg-slate-900/40 backdrop-blur-md p-4 animate-in fade-in duration-300">
          <div className="bg-white w-full max-w-sm rounded-[2.5rem] p-10 text-center shadow-2xl border border-slate-100">
            ...
            <p className="text-slate-400 text-[10px] font-black mb-8 uppercase tracking-widest leading-relaxed">Isso removerá todo o grupo relacionado.</p>
